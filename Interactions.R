...
SCORE_inter=data.frame('Y'=don$Y)
...
for(ii in 1:nb){
  ...
  ## Linéaire =================================================================
  tmp <- lm(Y~.,data=donA)
  SCORE_inter[bloc==ii,"lm"] <- predict(tmp,donT)
  
  print("STEP AIC")
  # AIC ========================================================================
  tmp_aic <- stats::step(tmp,trace=0)
  SCORE_inter[bloc==ii,"aic"] <- predict(tmp_aic,donT)
  
  print("STEP BIC")
  # BIC ========================================================================
  tmp_bic <- stats::step(tmp,trace=0,k=log(nrow(donA)))
  SCORE_inter[bloc==ii,"bic"] <- predict(tmp_bic,donT)
  ...
  SCORE_inter[bloc==ii,'ridge']=predict(ridge,newx=donXT,s='lambda.min')
  SCORE_inter[bloc==ii,"ridge1se"] <- predict(ridge,donXT,s="lambda.1se")
  
  SCORE_inter[bloc==ii,'lasso']=predict(lasso,newx=donXT,s='lambda.min')
  SCORE_inter[bloc==ii,"lasso1se"] <- predict(lasso,donXT,s="lambda.1se")
  
  SCORE_inter[bloc==ii,'elnet']=predict(elnet,newx=donXT,s='lambda.min')
  SCORE_inter[bloc==ii,"elas1se"] <- predict(elnet,donXT,s="lambda.1se")
  ...
  SCORE_inter[bloc==ii,"foret"] <- predict(foret,donT)
  ...
  SCORE_inter[bloc==ii, 'best foret']= predict(best.foret, donT)
  ...
  SCORE_inter[bloc==ii,"gbm tot"] <- predict(gb,donT)
  ...
  SCORE_inter[bloc==ii,"gbm opt"] <- predict(gbopt,donT)
  ...
  SCORE_inter[bloc==ii,"xgb"] <- predict(xgbopt,xgb_data_test)
...
sort((apply(SCORE_inter,2,erreur2,Y=SCORE_inter[,'Y'])[-1]))
...
RES <- data.frame(Y_obs = Y_FIN,model_pred_inter= Y_pred_fin)

erreur2 = function(X,Y){mean((X-Y)^2)}

model_pred_inter <- (apply(X = RES,MARGIN = 2,FUN = erreur2,Y=RES[,'Y_obs']))[-1]
