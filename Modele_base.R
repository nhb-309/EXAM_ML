...
#
# classif déséquilibrée == prendre le seuil naturel.
#

setwd('L:/REVISION/')
list.files()

# si on est en bord de grille : ↑ nb itérations ou augmenter la profondeur de l'arbre ou augmenter le facteur de frein
# comp: mco, step, ridge, elasticnet, lasso, arbre, foret, gbm, xgboost. Au total 9 méthodes.
...
RES <- data.frame(Y_obs = Y_FIN,model_pred_base= Y_pred_fin)

erreur2 = function(X,Y){mean((X-Y)^2)}

model_pred_base <- (apply(X = RES,MARGIN = 2,FUN = erreur2,Y=RES[,'Y_obs']))[-1]
